...
# class DataModule(pl.LightningDataModule):
#     def __init__(self, data_dir: str, batch_size: int = 32, num_workers: int = 0, seed: int = 42, special=None):
#         super().__init__()
#         self.data_dir = Path(data_dir)
#         self.batch_size = batch_size
#         self.num_workers = num_workers
...
#     def setup(self, stage=None):
#         self.dataset = Dataset_Large(self.data_dir, label="labels", special=self.special)

#         # Split session indices
#         session_indices = list(range(len(self.dataset)))
#         random.seed(self.seed)
#         random.shuffle(session_indices)

#         train_split = int(0.8 * len(session_indices))
#         val_split = int(0.1 * len(session_indices))
